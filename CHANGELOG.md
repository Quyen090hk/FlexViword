...
### 新增：视频内实时字幕叠加（免外挂播放器）
- 播放时把当前句以经典字幕样式（白色粗体 + 黑描边、底部居中）叠加渲染到视频画面内，
  与转写时间戳实时同步——看视频读字幕不再需要外挂播放器加载字幕文件
- 控制条新增 CC 开关按钮（快捷键 C），设置页外观区同步开关；开启时下方字幕条自动
  隐藏文字避免重复，关闭则回到字幕条模式；偏好持久化（settings.subtitleOverlay）
- 纯音频任务自动隐藏叠加层（无可叠加画面），i18n 双语覆盖
