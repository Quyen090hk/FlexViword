...
| # | 亮点 | 问题 → 方案 | 面试官可能追问 |
| --- | --- | --- | --- |
| 1 | 引擎抽象（依赖倒置） | 三种转写路径（mock/浏览器/桌面）→ 统一接口 + 注册表 + 解析器（显式 > auto > 降级并提示原因）；跨端兼容性矩阵化审计：模型参数透传断裂（Go 硬编码 → 签名透传）、桌面 SiliconFlow 由 wails 引擎覆盖（标记 web-only 而非必挂）、取消语义按引擎能力分级（web 即时中断 / 桌面 race 让队列不阻塞） | 为什么不是简单 if/else；跨引擎行为一致性怎么测（平台 mock + 契约测试）；"静默不兼容"如何被矩阵审计暴露 |
